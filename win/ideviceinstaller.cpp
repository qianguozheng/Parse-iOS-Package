...
int pngnormal(char *filename, char *oldpng, long int size)
{
	...
	while(new_len < old_len)
	{
			...
			chunkType[2] == 'A' && chunkType[3] == 'T')
		{
			...
			chunkCRC = crc32(0, (const Bytef *)chunkType, 4);
			chunkCRC = crc32(chunkCRC, (const Bytef *)compressed_data, output_len);
...
static void plist_dict_print_to_stream(plist_t node, int* indent_level, FILE* stream)
{
	...
	while (subnode)
	{
		...
		free(key);
		key = NULL;
		//plist_node_print_to_stream(subnode, indent_level, (FILE*)stream);
		plist_dict_next_item(node, it, &key, &subnode);
	}
	free(it);
}
...
int extract(char * file)
{
	...
	sprintf(appicon, "%s%s", fulldir, bundleicons);
...
NOT_FOUND:
		...
		if (bundleicons && (NULL == strstr(bundleicons, ".png") && 
			NULL == strstr(bundleicons, ".PNG")))
		{
			//bundleicons = realloc(bundleicons, strlen(bundleicons)+5);
			//snprintf(bundleicons, strlen(bundleicons)+5, "%s.png", bundleicons);
...
RETRY:
	...
	/* Make the *.png file from IPA to visiable in Non-IPA format */
	/*char cmd[256];
